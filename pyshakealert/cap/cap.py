...
def eventToCap(
    ...
    :param int minmmi: Minimum MMI to send a CAP alert for
        (default is smallest)
    ...
    if status not in allowed_cap_status:
        raise ValueError(f'Invalid status {status}, must be \
{", ".join(allowed_cap_status)}')

    # Extrac the single contour information we need to send an alert,
    # we simply get the contour with the matching MMI and extract
    # its polygon information.
    logging.info(f'Searching for smaller MMI contout >= {minmmi}')
    mycontour = None
    for contour in event.gm.contours:
        if contour.mmi.value < minmmi:
            logging.debug(
                f'Contour MMI {contour.mmi.value} < {minmmi} - ignore')
            # too small to send a mesage
            continue
        elif contour.mmi.value == minmmi:
            logging.debug(
                f'Contour MMI {contour.mmi.value} = {minmmi} - found')
            # we found our exact contour
            mycontour = contour
            break
        elif mycontour is None or mycontour.mmi.value > contour.mmi.value:
            logging.debug(
                f'Contour MMI {contour.mmi.value} > {minmmi}')
            mycontour = contour
    if not mycontour:
        raise ValueError(f'There may no contour in the event message or \
none are below are above the MMI of {minmmi}')
    ...
    logging.info(f'Finding english census zones the intersect {polygon}')
    zone = census.get_census_divisions(polygon)
    if zone.empty:
        raise NoCensusZoneFound(
            f'Could not find any english census zone for {polygon}')
    logging.info(f'Found the following matches: {zone}')

    logging.info(f'Generate CAP using template {CAP_TEMPLATE}')
